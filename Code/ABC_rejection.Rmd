...
pacman::p_load(dplyr,ggplot2,tidyr,janitor,vroom,hrbrthemes,deSolve,furrr,ggpurb)
dyn.load("testODE.so")
```
...
num_at_1=sim[(precision*0.1/tmax),2]
...
#sim=odeint(ode_model,initial_contamination,times,args=(r,C,d,g,l))
#parameters=c(r,C,d,g,l)
  # parms <- c(C = 33, d = 100, g=2,r=37);
Y <-c(Contamination=initial_contamination) ;
# sim<- ode(initial_contamination, times, ode_model, parameters, method = "radau",atol = 1e-4, rtol = 1e-4)
# sim <- ode(Y, times, func = "derivs", parms = parameters, dllname = "decay_ODE", method = "radau",
#            initfunc = "initmod", atol = 1e-4, rtol = 1e-4)
sim <- ode(initial_contamination, times, func = "derivs", dllname = "testODE", initfunc = "initmod",
...
return(cbind(num_at_2,num_at_4,num_at_6))
# return(cbind(num_at_2,num_at_4,num_at_6,num_at_16,num_at_18,num_at_20,num_at_22,num_at_24))
}
```
...
```{r}
p<-ode(initial_contamination, times, ode_model, c(r=100,C=10555,d=5055,g=55,l=90), method = "radau",atol = 1e-4, rtol = 1e-4)

one_run=deterministic_run(precision,initial_contamination,c(r=100,C=10555,d=5055,g=55,l=90)) %>% as_tibble()
one_run<-cbind(c(2,4,6,16,18,20,22,24),t(one_run)) %>% as_tibble()
ggplot()+
  geom_line(data=p %>% as_tibble(),aes(x=time,y=Contamination))+
   geom_point(data=df %>% filter(treatment=="Treated"),aes(x=time,y=conc),alpha=0.5)+
    geom_point(data=one_run,aes(x=V1,y=V2),colour="red",shape=24,size=4)
  # geom_point(data=tibble(x=c(2,4,6,16,18,20,22,24),y=experimental_data),aes(x=x,y=y))
```
...
# TODO Modelar segundo cacho y ver si r explica
