...
Distance<-function(x,y,s){  # computes the Euclidean distance between two lists of the same length
  if (length(x) == length(y)){
    # sqrt(sum(((x)-(y))/(s))^2) #sqrt is very slow
    sum(((x)-(y))/(s))^2
  }
  else{
...
df %>% 
  ...
  filter(treatment=="Treated" & time<10) ->data
```
...
initial_contamination=c(Contamination=20550)#data$M[1]) #median 34
...
#475331#dictates distance
euclidean_distance = abs(dist(rbind(one_run,experimental_data))) #Distance(one_run, experimental_data,s) #
...
# TODO Modelar segundo cacho y ver si r explica el incrementa
