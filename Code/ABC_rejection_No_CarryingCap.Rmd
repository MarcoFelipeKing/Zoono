...
if(Sys.info()["machine"]=="arm64"){
  system("R CMD SHLIB ODE_No_CarryingCap_arm64.c")
  dyn.load("ODE_No_CarryingCap_arm64.so")  
}else{
...
dynLoad("ODE_No_CarryingCap_arm64")
...
# Define the ODE to solve - We can consider one of the following
...
ode_model<- function(t, y, parameters){
  with(
    as.list(c(y, parameters)),{
      dContamination <- Contaminationr -d * exp(-g * t) * Contamination
      return(list(dContamination))
    }
  )
}
...
## Deterministic run function - Non-C
...
df %>% 
  ...
  filter(treatment=="Treated" ) ->data #& time<10
```
...
initial_contamination=c(Contamination=20560)#data$M[1]) #median 34
...
delta=0.01#0.006136007

# if(NROW(parameter_sample)==50){
#   delta = quantile(distances,0.01)[1] %>% as.numeric()
#   print(paste("Reducing delta to ", delta))
# } else {delta=delta}

euclidean_distance = Distance(one_run, experimental_data,s) # abs(dist(rbind(one_run,experimental_data))) #
